---
name: agent-committee-orchestration
description: "Use bounded specialist committees under a Chairman."
version: 1.2.0
...
metadata:
  hermes:
    ...
    related_skills: [sol-terra-parallel-orchestration, hermes-agent]
---

# Agent Committee Orchestration

## Current model policy

For this default profile, Sol–Terra terminology is literal: GPT-5.6 Sol is the controller and Chairman at fixed `max` reasoning, while execution leaves use GPT-5.6 Luna at `max`. A Chairman takeover fences the prior worker and returns the bounded scope to Sol/max in Direct topology. Full Sol takeover prohibits committee/subagent dispatch for that task; disclose any required external review gate rather than silently bypassing it. This interpretation changes model roles only, not existing ownership, evidence, review, or safety gates.

Use this class-level skill when a primary Hermes profile needs a small, named group of specialist profiles to improve a material decision. A committee is an optional advisory communication layer over ordinary Hermes profiles and the canonical Bot Chat. It is not a new model router, execution engine, transcript store, or approval authority.

The primary profile is the **Chairman**. The Chairman owns scope, decomposition, dependency ordering, topology selection, conflict resolution, approvals, integration, independent verification, and the final decision. The Chairman follows the active fixed-max controller policy: GPT-5.6 Sol runs at `max`; verify the live profile and model-specific reasoning override before claiming the level is active. Specialists provide bounded evidence to the Chairman; they are not equal coordinators.
...
The Chairman independently verifies every material finding before accepting it. Verification should repeat or inspect the relevant evidence, check citations, read back changed resources, and reconcile disagreements. Committee consensus is never sufficient proof. The Chairman records accepted evidence, dissent, unavailable members, decisions, and any re-plan.
